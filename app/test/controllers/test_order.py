...
def test_create(app, ingredients, size, client_data, beverages):
    ...
    for param, value in order.items():
        ...
        ingredients_in_detail = set(item['ingredient']['_id'] for item in created_order['ingradient_detail'])
        pytest.assume(not ingredients_in_detail.difference(ingredient_ids))

        beverage_in_detail = set(item['beverage']['_id'] for item in created_order['beverage_detail'])
        pytest.assume(not beverage_in_detail.difference(beverages_ids))
...
def test_get_all(app, ingredients, sizes, client_data, beverages):
    created_sizes, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(ingredients, sizes, beverages)
    created_orders = []
    for _ in range(5):
...
def test_create_fail_client_data(app, ingredients, size, client_data, beverages):
    created_size, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(ingredients, [size], beverages)
    del client_data['client_phone']
    order = __order(created_ingredients, created_size, created_beverages, client_data)
    created_order, error = OrderController.create(order)
    pytest.assume(error is None)
    assert created_order == 'Invalid order payload'

def test_create_fail_size(app, ingredients, size, client_data, beverages):
    created_size, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(ingredients, [size], beverages)
    created_size['_id'] = '123456'
    order = __order(created_ingredients, created_size, created_beverages, client_data)
    created_order, error = OrderController.create(order)
    pytest.assume(error is None)
    assert created_order == 'Invalid size for Order'
